# Code _ feedback with required editing from Norhan 

## Please! Write the code just in English for reproducibility purposes 
##############################
...
# 2️⃣ Load data 
## Balqees: Here, you uploaded the wrong file; it should be counts_data_filtered.rds
...
# 6️⃣ Run DESeq2
dds <- DESeq(dds)
### Balqees:  WE do not contrast, remove primary and metastatic
res <- results(dds, contrast = c("sample_type", "Primary", "Metastatic"))  
...
### BALQEES: Why did you add the Tissue of the organ in the design already? SV1+SV2 will take the batch work! adjusted please 
...
## BALQEES:  Our factor is not the tissue_or_organ_of_origin but the sample_type(Normal/Tumor)
...
## why the contrast !?
# ترتيب النتائج بحسب 
res <- results(dds, contrast = c("condition", "Tumor", "Normal"))adjusted p-value
