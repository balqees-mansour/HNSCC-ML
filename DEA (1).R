...
# 2️⃣ Load data
counts <- read.delim("~/Downloads/new task/TCGA_HNSC_Raw_Counts_Matrix.tsv", row.names = 1, check.names = FALSE) # Here you upload the wrong file it should be counts_data_filtered.rds
...
dds <- DESeqDataSetFromMatrix(countData = counts,
                              colData = metadata,
                              design = ~ origin_of_tissue + sample_type)  # sample_type = Primary/Metastatic #WE WANT TO EXCLUDE THE METASTAIC DATA! and we want the design just on the sampletype 
...
# 6️⃣ Run DESeq2
dds <- DESeq(dds)
res <- results(dds, contrast = c("sample_type", "Primary", "Metastatic"))  # WE do not to contrast remove primary and metastatic
...
# 🔟 PCA plot for outlier detection # Should be before the differential 
...
keep <- rowSums(counts(dds) >= 10) >= 5 # Why did you use 5 
dds <- dds[keep, ]
dim(dds)
...
dds <- DESeqDataSetFromMatrix(
  countData = counts_data_filtered,
  colData = colData_new,
  design = ~ tissue_or_organ_of_origin + SV1 + SV2 + condition # Why you added the Tissue of organ in the design already SV1+SV2 will take the batch work ! adjusted please 
)

# تأكدي أنtissue_or_organ_of_origin عامل عاملية
dds$tissue_or_organ_of_origin <- factor(dds$tissue_or_organ_of_origin) # Our factor is not the tissue_or_organ_of_origin but the sample_type(Normal/Tumor)


dds <- DESeq(dds, test = "Wald")# why this 
res <- results(dds, contrast = c("condition", "Tumor", "Normal"))# why the contrast !?
